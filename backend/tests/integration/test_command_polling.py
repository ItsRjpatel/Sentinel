...
def agent_auth_headers(test_endpoint):
    ...
    token = create_access_token(
        subject=str(test_endpoint.id), username=test_endpoint.hostname, roles=[]
    )
...
def admin_auth_headers(admin_user):
    token = create_access_token(
        subject=str(admin_user.id), username=admin_user.username, roles=["admin"]
    )
    return {"Authorization": f"Bearer {token}"}


@pytest.mark.asyncio
async def test_poll_no_command(
    client: AsyncClient, db_session, test_endpoint, agent_auth_headers
):
...
@pytest.mark.asyncio
async def test_poll_one_command_becomes_sent(
    client: AsyncClient,
    db_session,
    test_endpoint,
    admin_auth_headers,
    ...
    create_resp = await client.post(
        "/api/v1/commands", json=payload, headers=admin_auth_headers
    )
