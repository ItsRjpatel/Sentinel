...
async def test_os_inventory_flow(client: AsyncClient, db_session):
    ...
    token = create_access_token(
        subject=str(endpoint_id), username="TEST-OS-HOST", roles=[]
    )
    ...
    post_resp = await client.post(
        "/api/v1/inventory/os", json=payload, headers=auth_headers
    )
    ...
    get_resp = await client.get("/api/v1/inventory/os", headers=auth_headers)
    ...
    get_by_id_resp = await client.get(
        f"/api/v1/inventory/os/{endpoint_id}", headers=auth_headers
    )
    ...
    post_invalid = await client.post(
        "/api/v1/inventory/os", json=payload_invalid, headers=auth_headers
    )
