...
async def test_software_inventory_flow(client: AsyncClient, db_session):
    ...
    token = create_access_token(
        subject=str(endpoint_id), username="TEST-SW-HOST", roles=[]
    )
    ...
    post_resp = await client.post(
        "/api/v1/inventory/software", json=payload, headers=auth_headers
    )
    ...
    stmt_sw = select(SoftwareInventory).where(
    ...
    post_rec = await client.post(
        "/api/v1/inventory/software", json=payload_reconcile, headers=auth_headers
    )
    ...
    get_resp = await client.get("/api/v1/inventory/software", headers=auth_headers)
    assert get_resp.status_code == 200
    assert len(get_resp.json()["data"]) == 2

    # 7. GET software inventory by endpoint_id
    get_by_id_resp = await client.get(
        f"/api/v1/inventory/software/{endpoint_id}", headers=auth_headers
    )
