...
async def admin_token(db_session):
    ...
    token = create_access_token(
        subject=str(admin.id), username=admin.username, roles=["admin"]
    )
    return token
...
async def test_queue_command(
    client: AsyncClient, db_session, test_endpoint, auth_headers
):
    payload = {
...
@pytest.mark.asyncio
async def test_queue_duplicate_inventory_command(
    client: AsyncClient, db_session, test_endpoint, auth_headers
):
...
async def test_cancel_command(
    ...
    cancel_response = await client.patch(
        f"/api/v1/commands/{command_id}/cancel", headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_get_endpoint_commands(
    client: AsyncClient, db_session, test_endpoint, auth_headers
):
    ...
    response = await client.get(
        f"/api/v1/endpoints/{test_endpoint.id}/commands", headers=auth_headers
    )
    assert response.status_code == 200
