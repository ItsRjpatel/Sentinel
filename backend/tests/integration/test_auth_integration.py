...
async def test_jwt_expired_token(client):
    ...
    assert res_data["success"] is False
    assert (
        "expire" in res_data["message"].lower()
        or "expired" in res_data["message"].lower()
    )
...
async def test_db_cascade_deletes(db_session, setup_test_roles_perms):
    ...
    user = User(
        username="cascade_user", email="cascade@example.com", password_hash="hash"
    )
    db_session.add(user)
    await db_session.flush()
    ...
    token = RefreshToken(
        token_hash="somehashvalue", expiry=datetime.now(timezone.utc), user_id=user.id
    )
    ...
    # Verify items exist
    r_token = await db_session.execute(
        text("SELECT COUNT(*) FROM refresh_tokens WHERE user_id = :uid"),
        {"uid": user.id},
    )
    assert r_token.scalar() == 1

    r_role = await db_session.execute(
        text("SELECT COUNT(*) FROM user_roles WHERE user_id = :uid"), {"uid": user.id}
    )
    ...
    r_token_after = await db_session.execute(
        text("SELECT COUNT(*) FROM refresh_tokens WHERE user_id = :uid"),
        {"uid": user.id},
    )
    assert r_token_after.scalar() == 0

    r_role_after = await db_session.execute(
        text("SELECT COUNT(*) FROM user_roles WHERE user_id = :uid"), {"uid": user.id}
    )
...
async def test_bootstrap_idempotency_real_db(db_session):
    ...
    # Get current counts
    perm_count_1 = (
        await db_session.execute(text("SELECT COUNT(*) FROM permissions"))
    ).scalar()
    role_count_1 = (
        await db_session.execute(text("SELECT COUNT(*) FROM roles"))
    ).scalar()
...
async def test_bootstrap_non_destructive_real_db(db_session, setup_test_roles_perms):
    ...
    admin_res = await db_session.execute(
    ...
    admin_after = await db_session.execute(
        text("SELECT password_hash FROM users WHERE username = 'admin'")
    )
    assert (
        admin_after.scalar() == modified_hash
    ), "Admin credentials must not be reset by bootstrap seeding"
