...
async def main():
    async with httpx.AsyncClient(base_url='http://127.0.0.1:8000') as client:
        ...
        # Call Login
        response = await client.post('/api/v1/auth/login', json={'username': 'admin', 'password': 'TestPassword123!'})
        ...
        if response.status_code == 200:
            ...
            async with async_session_maker() as session:
                result = await session.execute(text('SELECT COUNT(*) FROM refresh_tokens WHERE revoked=false'))
                print('Active Refresh tokens count after refresh:', result.scalar())
                result = await session.execute(text('SELECT COUNT(*) FROM refresh_tokens WHERE revoked=true'))
        ...
        if response.status_code == 200:
            ...
            logout_res = await client.post('/api/v1/auth/logout', json={'refresh_token': refresh_token}, headers={'Authorization': f'Bearer {access_token}'})
