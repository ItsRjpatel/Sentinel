...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, username: str) -> Connection:
        ...
        logger.info(f"WebSocket client connected: {username}. Total connections: {len(self.active_connections)}")
        return connection

    def disconnect(self, connection: Connection):
        if connection in self.active_connections:
            self.active_connections.remove(connection)
            logger.info(f"WebSocket client disconnected: {connection.username}. Total connections: {len(self.active_connections)}")
    ...
    async def _send_safe(self, connection: Connection, message: dict):
        ...
        try:
            ...
            logger.warning(f"Failed to send to {connection.username}, marking inactive: {e}")
            connection.active = False
            self.disconnect(connection)

    async def broadcast(self, event: WebSocketEvent):
        message = event.model_dump()
        tasks = [self._send_safe(conn, message) for conn in list(self.active_connections)]
        if tasks:
    ...
    async def cleanup_dead_connections(self):
        ...
        while True:
            ...
            logger.debug(f"Running WebSocket cleanup. Active connections: {len(self.active_connections)}")
            for conn in list(self.active_connections):
                try:
