...
class CommandService:
    ...
    async def queue_command(self, cmd_in: CommandCreate) -> Command:
        ...
        if cmd_in.expires_in_seconds:
            expires_at = self._get_utc_now() + timedelta(
                seconds=cmd_in.expires_in_seconds
            )
        ...
        logger.info(
            f"[COMMAND LIFECYCLE: QUEUED] Command {created_cmd.id} queued for Endpoint {created_cmd.endpoint_id} ({created_cmd.command_type})"
        )
    ...
    async def queue_bulk_commands(
        self, bulk_in: BulkCommandCreate, created_by: str
    ) -> BulkCommandResponse:
        ...
        if bulk_in.expires_in_seconds:
            expires_at = self._get_utc_now() + timedelta(
                seconds=bulk_in.expires_in_seconds
            )
        ...
        for c in saved_commands:
            logger.info(
                f"[COMMAND LIFECYCLE: BULK QUEUED] Command {c.id} queued for Endpoint {c.endpoint_id} ({c.command_type})"
            )
            event_dispatcher.publish(
                "COMMAND_QUEUED",
                {
    ...
    async def retry_command(self, command_id: UUID, created_by: str) -> Command:
        ...
        logger.info(
            f"[COMMAND LIFECYCLE: RETRIED] Retried Command {original.id} -> New Command {new_cmd.id}"
        )
        return new_cmd
    ...
    async def cancel_command(self, command_id: UUID) -> Command:
        ...
        logger.info(
            f"[COMMAND LIFECYCLE: CANCELLED] Command {cancelled.id} marked CANCELLED"
        )
    ...
    async def poll_command(self, endpoint_id: UUID) -> Optional[Command]:
        ...
        command = await self.repository.get_oldest_pending_for_endpoint_and_lock(
            endpoint_id
        )
        if not command:
            return None
        ...
        if command.expires_at and self._get_utc_now() > command.expires_at:
            ...
            logger.warning(
                f"[COMMAND LIFECYCLE: TIMEOUT] Command {timed_out.id} expired prior to poll"
            )
            event_dispatcher.publish(
                "COMMAND_TIMEOUT",
                {
        ...
        logger.info(
            f"[COMMAND LIFECYCLE: SENT] Command {sent_cmd.id} ({sent_cmd.command_type}) sent to Endpoint {endpoint_id}"
        )
    ...
    async def get_endpoint_commands(
        ...
        return await self.repository.list_by_endpoint(
            endpoint_id, skip=skip, limit=limit
        )
    ...
    async def update_command_result(
        ...
        updated_cmd = await self.repository.update_result(
            command=command, success=success, result=result, error_message=error_message
        )
        logger.info(
            f"[COMMAND LIFECYCLE: {updated_cmd.status}] Command {command_id} result uploaded: Success={success}"
        )

        event_dispatcher.publish(
            f"COMMAND_{updated_cmd.status}",
            {
