...
class ScheduledJob(Base, BaseModelMixin):
    ...
    job_type = Column(String(50), nullable=False) # INVENTORY, COMMAND, POLICY_REFRESH, HEARTBEAT_CHECK, CLEANUP
    schedule_type = Column(String(50), nullable=False, default="RECURRING") # RECURRING or ONE_TIME
    cron_expression = Column(String(100), nullable=True) # E.g. "0 */6 * * *"
    next_run_at = Column(DateTime, nullable=True, index=True)
    last_run_at = Column(DateTime, nullable=True)
    status = Column(String(50), nullable=False, default="ACTIVE") # ACTIVE, PAUSED, COMPLETED, EXPIRED
...
class JobExecutionHistory(Base, BaseModelMixin):
    ...
    job_id = Column(String(36), ForeignKey("scheduled_jobs.id", ondelete="CASCADE"), nullable=False, index=True)
