...
class Alert(Base, BaseModelMixin):
    ...
    alert_type: Mapped[str] = mapped_column(String(100), nullable=False, default="custom")
...
class AlertRule(Base, BaseModelMixin):
    ...
    alert_type: Mapped[str] = mapped_column(String(100), nullable=False, unique=True, index=True)
    is_enabled: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    trigger_threshold: Mapped[float] = mapped_column(Float, nullable=False, default=90.0)
    trigger_duration_samples: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    resolve_threshold: Mapped[float] = mapped_column(Float, nullable=False, default=80.0)
    resolve_duration_samples: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    severity: Mapped[str] = mapped_column(String(50), nullable=False, default="high")
    category: Mapped[str] = mapped_column(String(100), nullable=False, default="Performance")
...
class EndpointAlertState(Base, BaseModelMixin):
    ...
    endpoint_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("endpoints.id", ondelete="CASCADE"), nullable=False, index=True)
    alert_type: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    consecutive_trigger_samples: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    consecutive_resolve_samples: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
