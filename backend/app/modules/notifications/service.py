...
from app.modules.notifications.schemas import NotificationCreate, NotificationPreferenceSchema
...
class NotificationService:
        ...
        if data.user_id:
            ...
            if pref:
                if pref.email_enabled and pref.email_address:
                    await NotificationChannelSender.send_email(pref.email_address, data.title, data.message)
                if pref.webhook_enabled and pref.webhook_url:
                    await NotificationChannelSender.send_webhook(pref.webhook_url, {"title": data.title, "message": data.message})
                if pref.slack_enabled and pref.slack_webhook_url:
                    await NotificationChannelSender.send_slack(pref.slack_webhook_url, data.title, data.message)
    ...
    async def save_preferences(self, user_id: str, data: NotificationPreferenceSchema) -> NotificationPreferenceSchema:
