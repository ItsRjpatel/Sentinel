...
class PolicyService:
    ...
    async def create_policy(
        self, data: PolicyCreate, user_name: Optional[str] = None
    ) -> Policy:
        policy = Policy(
            name=data.name,
    ...
    async def rollback(
        self, policy_id: str, version_number: int, user_name: Optional[str] = None
    ) -> Optional[Policy]:
        versions = await self.repo.get_versions(policy_id)
        target_version = next(
            (v for v in versions if v.version == version_number), None
        )
        ...
        return await self.repo.update(
            policy, f"Rolled back to version {version_number}"
        )

    async def clone_policy(
        self, policy_id: str, new_name: str, user_name: Optional[str] = None
    ) -> Optional[Policy]:
    ...
    async def check_conflicts(
        self, target_type: str, target_ids: List[str], category: str
    ) -> PolicyConflictInfo:
