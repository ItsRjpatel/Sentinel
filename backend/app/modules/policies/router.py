...
async def rollback_policy(
    ...
    service = PolicyService(db)
    rolled_back = await service.rollback(policy_id, version_number, current_user.username)
    if not rolled_back:
        raise HTTPException(status_code=400, detail="Rollback failed. Invalid version number or policy.")
    return rolled_back
...
async def assign_policy(
    ...
    service = PolicyService(db)
    await service.assign_policy(policy_id, payload.target_type, payload.target_ids, current_user.username)
...
async def check_policy_conflicts(
    ...
    service = PolicyService(db)
    return await service.check_conflicts(payload.target_type, payload.target_ids, category)
