...
async def test_authenticate_success(auth_service: AuthenticationService) -> None:
    ...
    mock_user = User(id=user_id, username="admin", password_hash=hashed, is_active=True)
...
@pytest.mark.asyncio
async def test_authenticate_invalid_password(auth_service: AuthenticationService) -> None:
    user_id = uuid.uuid4()
...
async def test_login(mock_create_token: AsyncMock, auth_service: AuthenticationService) -> None:
    user_id = uuid.uuid4()
    password = "SecurePassword123!"
    hashed = get_password_hash(password)
    mock_user = User(id=user_id, username="admin", password_hash=hashed, is_active=True)
...
@pytest.mark.asyncio
@patch("app.modules.auth.service.create_access_token")
async def test_refresh_session_success(mock_create_token: AsyncMock, auth_service: AuthenticationService) -> None:
    user_id = uuid.uuid4()
    mock_user = User(id=user_id, username="admin", is_active=True)
    token_record = RefreshToken(user_id=user_id, token_hash="hash", revoked=False, expiry=datetime.now(timezone.utc))
    # Make token valid
    token_record.expiry = datetime.now(timezone.utc) + datetime.timedelta(days=1) if hasattr(datetime, "timedelta") else datetime.fromtimestamp(datetime.now(timezone.utc).timestamp() + 86400, timezone.utc)
...
async def test_refresh_session_revoked(auth_service: AuthenticationService) -> None:
    user_id = uuid.uuid4()
    token_record = RefreshToken(user_id=user_id, token_hash="hash", revoked=True, expiry=datetime.now(timezone.utc))
    auth_service.refresh_token_repo.get_by_hash = AsyncMock(return_value=token_record)  # type: ignore
