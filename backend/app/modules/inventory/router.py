...
def get_windows_update_repository(
    session: AsyncSession = Depends(get_db),
) -> WindowsUpdateInventoryRepository:
...
def get_windows_service_repository(
    session: AsyncSession = Depends(get_db),
) -> WindowsServiceInventoryRepository:
...
async def _resolve_authenticated_endpoint(token: str, db: AsyncSession) -> uuid.UUID:
    ...
    if not sub:
        raise HTTPException(
            status_code=401, detail="Missing subject parameter in session."
        )
...
@router.post(
    "/inventory/hardware", response_model=SuccessResponse[HardwareInventoryResponse]
)
...
async def get_my_hardware(
    ...
    if not record:
        raise HTTPException(
            status_code=404, detail="No hardware inventory record found."
        )
    return SuccessResponse(
...
@router.post(
    "/inventory/os", response_model=SuccessResponse[OperatingSystemInventoryResponse]
)
async def upload_os(
...
@router.get(
    "/inventory/os/{endpoint_id}",
    response_model=SuccessResponse[OperatingSystemInventoryResponse],
)
async def get_endpoint_os(
    ...
    if not record:
        raise HTTPException(
            status_code=404, detail="No inventory record matched for target ID."
        )
    return SuccessResponse(
        message="OS inventory retrieved",
        data=OperatingSystemInventoryResponse.model_validate(record),
...
@router.post(
    "/inventory/network",
    response_model=SuccessResponse[list[NetworkAdapterInventoryResponse]],
)
...
@router.post(
    "/inventory/storage",
    response_model=SuccessResponse[list[PhysicalDiskInventoryResponse]],
)
...
@router.post(
    "/inventory/software",
    response_model=SuccessResponse[list[SoftwareInventoryResponse]],
)
...
@router.get(
    "/inventory/software/{endpoint_id}",
    response_model=SuccessResponse[list[SoftwareInventoryResponse]],
)
...
@router.post(
    ...
    response_model=SuccessResponse[list[WindowsUpdateInventoryResponse]],
)
...
@router.get(
    "/inventory/windows-updates/{endpoint_id}",
    response_model=SuccessResponse[list[WindowsUpdateInventoryResponse]],
)
...
@router.post(
    "/inventory/services",
    response_model=SuccessResponse[list[WindowsServiceInventoryResponse]],
)
...
@router.get(
    "/inventory/services/{endpoint_id}",
    response_model=SuccessResponse[list[WindowsServiceInventoryResponse]],
)
...
@router.get(
    ...
    response_model=SuccessResponse[HardwareInventoryResponse],
)
@router.get(
    "/inventory/hardware/{endpoint_id}",
    response_model=SuccessResponse[HardwareInventoryResponse],
)
