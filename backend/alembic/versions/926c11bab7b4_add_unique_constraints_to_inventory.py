...
def downgrade() -> None:
    op.drop_constraint(
        "uq_windows_service_endpoint_name", "windows_service_inventory", type_="unique"
    )
    op.drop_constraint(
        "uq_windows_update_endpoint_kb", "windows_update_inventory", type_="unique"
    )
    op.drop_constraint(
        "uq_software_endpoint_app_pub_ver", "software_inventory", type_="unique"
    )
    op.drop_constraint(
        "uq_logical_volume_disk_guid", "logical_volume_inventory", type_="unique"
    )
    op.drop_constraint(
        "uq_physical_disk_endpoint_serial", "physical_disk_inventory", type_="unique"
    )
    op.drop_constraint(
        "uq_network_adapter_endpoint_guid", "network_adapter_inventory", type_="unique"
    )
